...
public class VersionManager<T extends BaseVersionedNetworkContent> {
	...
	private NavigableMap<Number160, T> buildData(Map<PeerAddress, Map<Number640, Data>> rawData) {
			...
			if (tmp == null || tmp.isEmpty()) {
				...
				for (Number640 key : tmp.keySet()) {
					try {
						...
						if (object instanceof BaseVersionedNetworkContent) {
							dataMap.put(key.versionKey(), (T) tmp.get(key).object());
						} else {
