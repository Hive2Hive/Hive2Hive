...
public class GetNextFromQueueMessage extends RequestUserMessage {
	...
	public synchronized void run() {
		...
		UserMessageQueue umQueue = (UserMessageQueue)networkManager.getLocal(locationKey, H2HConstants.USER_MESSAGE_QUEUE_KEY);
		Queue<UserMessage> messageQueue = umQueue.getMessageQueue();
		
		// response object
		NextFromQueueResponse responseContent = new NextFromQueueResponse(messageQueue.poll(), messageQueue.size());
